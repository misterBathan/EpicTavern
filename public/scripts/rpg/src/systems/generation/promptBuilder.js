 ...
 * @param {Array<'userStats'|'infoBox'|'characters'>|null} [sections=null] - If set, only request these tracker sections
 * @returns {string} Formatted instruction text for the AI
 */
export function generateTrackerInstructions(includeHtmlPrompt = true, includeContinuation = true, includeAttributes = true, sections = null) {
    const userName = getContext().name1;
    const classicStats = extensionSettings.classicStats;
    const trackerConfig = extensionSettings.trackerConfig;
    let instructions = '';

    const wantSection = (key) => !sections || sections.includes(key);

    // Check if any trackers are enabled
    const hasAnyTrackers = (extensionSettings.showUserStats && wantSection('userStats'))
        || (extensionSettings.showInfoBox && wantSection('infoBox'))
        || (isPresentCharactersEnabled() && wantSection('characters'));
    ...
    if (hasAnyTrackers) {
        ...
        // Universal instruction header — Together mode: narrative first, JSON last.
        // Asking for JSON first caused many models to emit only trackers (empty chat bubble after strip).
        if (includeContinuation) {
            instructions += '\nFirst write your complete in-character roleplay reply. Then, at the END of the same reply, append an update to the trackers in EXACTLY the JSON format shown below as a single unified JSON object containing all enabled tracker fields. ';
        } else {
            instructions += '\nProvide an update to the trackers in EXACTLY the JSON format shown below as a single unified JSON object containing all enabled tracker fields. ';
        }

        if (sections && sections.length > 0 && sections.length < 3) {
            instructions += `Only include these keys in the JSON object: ${sections.join(', ')}. Omit other tracker keys entirely (they will be preserved from previous state). `;
        }
        ...
        if (extensionSettings.showUserStats && wantSection('userStats')) {
            enabledTrackers.push('userStats');
        }
        if (extensionSettings.showInfoBox && wantSection('infoBox')) {
            enabledTrackers.push('infoBox');
        }
        if (isPresentCharactersEnabled() && wantSection('characters')) {
            enabledTrackers.push('characters');
        }

        if (enabledTrackers.length > 0) {
            if (includeContinuation) {
                instructions += '\n\nFORMAT:\n\nAfter your roleplay text, append EXACTLY ONE JSON code block with ALL tracker sections wrapped in a single object:\n\n```json\n{\n';
            } else {
        ...
        if (includeContinuation) {
            ...
            if (customPrompt) {
                ...
                instructions += `\n\nCRITICAL: Do not reply with only the JSON block. Your visible reply must be a full roleplay continuation from where the last message left off; put the JSON tracker block after that narrative. Ensure the trackers naturally reflect and influence the scene (fatigue, hygiene, weather, mood, etc.). Replace all bracketed placeholders (e.g., [Location], [Mood Emoji]) with actual content without square brackets.\n\n`;
            }
        }
 ...
 * Uses COMMITTED data (not displayed data) for generation context.
 *
 * @param {Array<'userStats'|'infoBox'|'characters'>|null} [sections=null] - Optional sectional update filter
 * @returns {string} Full prompt text for separate tracker generation
 */
export function generateRPGPromptText(sections = null) {
    // Use COMMITTED data for generation context, not displayed data
    const userName = getContext().name1;
    ...
    promptText += generateTrackerInstructions(false, false, false, sections);
 ...
 */
export async function generateSeparateUpdatePrompt(sections = null) {
    ...
    instructionMessage += generateRPGPromptText(sections).replace('start your response with', 'respond with');
