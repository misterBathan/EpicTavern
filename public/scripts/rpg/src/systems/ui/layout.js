...
/**
 * True when a character/group chat is actually open (not just the Chat screen).
 * @returns {boolean}
 */
export function hasActiveChatSession() {
    try {
...
export function togglePlotButtons() {
    if (!extensionSettings.enabled || !hasActiveChatSession()) {
 ...
 * Requires RPG enabled AND an open chat session.
 */
export function updatePanelVisibility() {
    const show = !!(extensionSettings.enabled && hasActiveChatSession());
    if (show) {
        $panelContainer?.show();
        ...
        $('body').removeClass('et-rpg-no-active-chat');
    } else {
        $panelContainer?.hide();
        ...
        $('body').addClass('et-rpg-no-active-chat');
    }
}
...
export function updateGenerationModeUI() {
    // Always keep refresh available — Together mode can fall back to a separate tracker call
    $('#rpg-manual-update').show();
    $('#rpg-strip-refresh').show();
    $('#rpg-header-refresh').show();

    if (extensionSettings.generationMode === 'together') {
        $('#rpg-external-api-settings').slideUp(200);
        $('#rpg-separate-mode-settings').slideUp(200);
        // Auto-update still applies to the Together→Separate fallback
