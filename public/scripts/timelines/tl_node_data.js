...
export async function fetchData(characterAvatar) {
    ...
    // API may return { error: true } when the character chats folder is missing
...
/**
 * Merge the currently open in-memory chat into the disk-fetched chat map.
 * Ensures the Timeline shows messages even when the file list is stale or empty.
 * @param {Object} chat_dict
 * @returns {Object}
 */
function mergeCurrentChatFromContext(chat_dict) {
    ...
    if (context.groupId) {
        ...
        fileName = group?.chat_id ? String(group.chat_id) : null;
    } else {
        fileName = context.getCurrentChatId?.() || context.chatId || null;
    }
    ...
    const key = String(fileName).endsWith('.jsonl') ? String(fileName) : `${fileName}.jsonl`;
    const existing = chat_dict[key];
    if (!existing || existing.length < messages.length) {
        chat_dict[key] = messages.slice();
    }
    return chat_dict;
}
...
export async function prepareData(data, isGroupChat) {
    const context = getContext();
    let chat_dict = {};
    const listSource = Array.isArray(data) ? data : (data && typeof data === 'object' ? Object.values(data) : []);
    let chat_list = listSource
        .filter(x => x && typeof x === 'object' && x.file_name)
        ...
        try {
            ...
            const requestBody = isGroupChat
                ...
                : JSON.stringify({
                    ...
                    file_name: String(file_name).replace('.jsonl', ''),
    ...
    chat_dict = mergeCurrentChatFromContext(chat_dict);
