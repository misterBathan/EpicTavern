...
/**
 * Extracts the alpha (opacity) value from a color string.
 * Hex / named colors (extension defaults) have no alpha channel → treat as fully opaque.
 *
 * @param {string} colorString - CSS color (`#rgb`, `#rrggbb`, `rgb(...)`, `rgba(...)`, etc.).
 * @returns {number} Opacity in [0, 1]. Defaults to 1 when alpha cannot be parsed.
 */
function getAlphaFromRGBA(colorString) {
    if (colorString == null || colorString === '') {
        return 1;
    }
    const asString = String(colorString).trim();
    const rgbaMatch = asString.match(/rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*(\d*(?:\.\d+)?)\s*\)/i);
    if (rgbaMatch) {
        const alpha = parseFloat(rgbaMatch[1]);
        return Number.isFinite(alpha) ? alpha : 1;
    }
    // Hex / rgb() without alpha / named colors → fully opaque
    return 1;
}
...
export function setupStylesAndData(nodeData) {
    ...
    const avatarImg = selected_group
        ? group?.avatar_url
        : (character?.avatar ? getThumbnailUrl('avatar', character.avatar) : null);
