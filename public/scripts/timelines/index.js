...
let defaultSettings = {
    ...
    avatarAsRoot: true,
    showLegend: true,
    bookmarkColor: 'rgba(255, 0, 0, 1)',
    useChatColors: false,
    charNodeColor: 'rgba(255, 255, 255, 1)',
    userNodeColor: 'rgba(173, 216, 230, 1)',
    edgeColor: 'rgba(200, 200, 200, 1)',
...
function initializeCytoscape(nodeData, styles) {
    ...
    if (typeof cytoscape !== 'function') {
        console.error('[EpicTavern] cytoscape is not loaded yet');
        return null;
    }

    try {
        if (typeof cytoscapeDagre !== 'undefined') cytoscape.use(cytoscapeDagre);
        if (typeof cytoscapeContextMenus !== 'undefined') cytoscape.use(cytoscapeContextMenus);
        if (typeof cytoscapePopper !== 'undefined') cytoscape.use(cytoscapePopper);
    } catch (error) {
        // Plugins may already be registered from a previous open
        console.debug('[EpicTavern] Cytoscape plugin register:', error);
    }

    // Destroy previous instance so reopen always draws fresh
    if (theCy) {
        try { theCy.destroy(); } catch (_) { /* ignore */ }
        theCy = null;
    }
    timelinesDiagramDiv.innerHTML = '';

    // Guarantee a non-zero drawing surface (Cytoscape silently draws nothing at 0×0)
    const host = timelinesDiagramDiv.parentElement || timelinesDiagramDiv;
    const hostRect = host.getBoundingClientRect();
    if (hostRect.width < 10 || hostRect.height < 10) {
        timelinesDiagramDiv.style.width = '100%';
        timelinesDiagramDiv.style.height = `${Math.max(320, window.innerHeight - 120)}px`;
    } else {
        timelinesDiagramDiv.style.width = '100%';
        timelinesDiagramDiv.style.height = '100%';
    }

    const layoutConfig = (typeof cytoscapeDagre !== 'undefined' && layout?.name === 'dagre')
        ? layout
        : { name: 'breadthfirst', directed: true, padding: 30, spacingFactor: 1.1 };
...
async function updateTimelineDataIfNeeded(forceReload = false) {
    ...
    const characterChanged = !lastContext
        || lastContext.characterId !== context.characterId
        || lastContext.groupId !== context.groupId;

    if (!forceReload && !characterChanged && lastTimelineData) {
        return false; // No update occurred
    }

    let data = {};

    // Prefer group mode only when a group is active and no solo character is selected
    const isGroupChat = Boolean(context.groupId) && (context.characterId === undefined || context.characterId === null);
    ...
    if (isGroupChat) {
        ...
        if (groupID) {
            ...
            lastTimelineData = await prepareData([], true);
        }
    }
    else {
        const avatar = context.characters?.[context.characterId]?.avatar;
        data = await fetchData(avatar);
        lastTimelineData = await prepareData(data, false);
    }

    lastContext = context; // Update `lastContext` to the current context
    console.info('Timeline data updated', {
        nodes: Array.isArray(lastTimelineData) ? lastTimelineData.length : 0,
...
/**
 * Wait until Cytoscape core (and preferably dagre layout) is available.
 * Tippy/popper/context-menus are optional for drawing the graph.
 * @param {number} timeoutMs
 * @returns {Promise<boolean>}
 */
async function waitForCytoscape(timeoutMs = 15000) {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
        if (typeof cytoscape === 'function') {
            // dagre is preferred but not strictly required to attempt a draw
            if (typeof cytoscapeDagre !== 'undefined' || Date.now() - start > 2000) {
                return true;
            }
        }
        await delay(50);
    }
    console.error('[EpicTavern] Timed out waiting for Cytoscape libraries', {
        cytoscape: typeof cytoscape,
        cytoscapeDagre: typeof cytoscapeDagre,
        cytoscapePopper: typeof cytoscapePopper,
        cytoscapeContextMenus: typeof cytoscapeContextMenus,
    });
    return false;
}
...
function zoomToCurrentChatNode(cy) {
    ...
    if (!newCenterNode || newCenterNode.length === 0) {
        cy.fit(undefined, 40);
        return;
    }
...
async function onTimelineButtonClick() {
    ...
    try {
        showLoader();
        // Always refresh so new messages in the open chat appear
        dataUpdated = await updateTimelineDataIfNeeded(true);
    } catch (error) {
        console.error('[EpicTavern] Timeline data load failed:', error);
        toastr?.error?.('Failed to load timeline data. Check the browser console.', 'Timeline');
    }
    finally {
    ...
    await delay(50); // let exclusive-screen CSS apply so the graph container has size

    const libsReady = await waitForCytoscape();
    if (libsReady && lastTimelineData) {
        try {
            ...
            if (theCy) {
                ...
                theCy.resize();
                theCy.fit(undefined, 40);
                console.info('[EpicTavern] Timeline graph ready', {
                    elements: theCy.elements().length,
                    nodes: theCy.nodes().length,
                });
            }
        } catch (error) {
            console.error('[EpicTavern] Timeline render failed:', error);
            toastr.error('Timeline graph failed to render. Check the browser console.', 'Timeline');
        }
    } else if (!libsReady) {
        toastr.error('Timeline graph library failed to load. Refresh the page and try again.', 'Timeline');
    } else {
        toastr.warning('No timeline data for this chat yet.', 'Timeline');
    }
    closeOpenDrawers();
...
/**
 * Boot Timelines as core EpicTavern UI (Timeline screen + Settings → Timeline).
 */
export async function initTimelines() {
    ...
    const settingsHost = document.getElementById('et-timeline-settings')
        ?? document.getElementById('timelines_container')
        ?? document.getElementById('extensions_settings');
    $(settingsHost).append(settingsHtml);
    $('#show_timeline_view').on('click', () => {
        navigate('timeline');
    });
    ...
    $('#toggleStyleSettings').on('click', function () {
        $('#styleSettingsArea').toggleClass('hidden');
    });
    $('#toggleColorSettings').on('click', function () {
    ...
    document.addEventListener('et-screen-changed', (event) => {
        ...
        if (screen === 'timeline' && previous !== 'timeline') {
            onTimelineButtonClick();
        } else if (previous === 'timeline' && screen !== 'timeline') {
            closeModal({ skipNavigate: true });
            closeTippy();
        }
    });
    ...
    console.info('[EpicTavern] Timelines ready');
}
