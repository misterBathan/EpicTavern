...
import { getCurrentScreen, navigate } from '../app-nav.js';
 ...
 * @param {{ skipNavigate?: boolean }} [options]
 */
export function closeModal(options = {}) {
    ...
    const storage = document.querySelector('.timelines-modal-storage');
    if (storage) {
        storage.appendChild(modal);
    }
    modal.style.display = 'none';

    if (!options.skipNavigate && getCurrentScreen() === 'timeline') {
        navigate('chat');
    }
}
